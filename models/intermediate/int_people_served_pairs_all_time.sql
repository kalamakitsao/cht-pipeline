-- models/intermediate/int_people_served_pairs_all_time.sql
{{ config(
    materialized='incremental',
    unique_key=['location_id', 'patient_id'],
    incremental_strategy='delete+insert',
    on_schema_change='ignore',
    indexes=[
      {'columns': ['location_id', 'patient_id'], 'unique': true},
      {'columns': ['last_seen_saved_timestamp']},
      {'columns': ['location_id']}
    ],
    tags=['people_served','cadence_daily']
) }}
