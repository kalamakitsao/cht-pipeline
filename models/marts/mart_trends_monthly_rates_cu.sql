...
monthly_base AS (
  SELECT LOWER(level) AS level, county, sub_county, community_unit, period_start, month_year, LOWER(metric_id) AS metric_id, value::numeric AS value
  FROM {{ ref('mart_household_regd_monthly_cu') }}
  UNION ALL
  SELECT LOWER(level), county, sub_county, community_unit, period_start, month_year, LOWER(metric_id), value::numeric
  FROM {{ ref('mart_household_visit_monthly_cu') }}
),

num_sums_std AS (
  SELECT m.metric_id, b.level, b.county, b.sub_county, b.community_unit, b.period_start, b.month_year, SUM(b.value) AS numerator_value
...
den_sums_std AS (
  SELECT m.metric_id, b.level, b.county, b.sub_county, b.community_unit, b.period_start, b.month_year, SUM(b.value) AS denominator_value
...
computed_std AS (
  SELECT ns.level, ns.county, ns.sub_county, ns.community_unit, ns.period_start, ns.month_year, ns.metric_id,
...
num_chp AS (
  SELECT 'monthly_rates_active_chp'::text AS metric_id, a.level, a.county, a.sub_county, a.community_unit, a.period_start, a.month_year, SUM(a.value) AS numerator_value
  FROM chp_activity a
...
den_chp AS (
  SELECT county, sub_county, community_unit, value::numeric AS chps_enrolled
  FROM {{ ref('mv_aggregate_metrics_by_community_unit') }}
  WHERE metric_id = 'chps_enrolled' AND period_id = 1
),

computed_chp AS (
  SELECT n.level, n.county, n.sub_county, n.community_unit, n.period_start, n.month_year, n.metric_id,
