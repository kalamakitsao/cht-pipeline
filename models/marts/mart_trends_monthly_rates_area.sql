{{ config(
  materialized = 'table',
  unique_key   = ['level','county','sub_county','community_unit','chp_area','period_start','metric_id'],
...
monthly_base AS (
  SELECT LOWER(level) AS level, county, sub_county, community_unit, chp_area, period_start, month_year,
         LOWER(metric_id) AS metric_id, value::numeric AS value
  FROM afyabi.mart_household_regd_monthly_area
  UNION ALL
  SELECT LOWER(level), county, sub_county, community_unit, chp_area, period_start, month_year,
         LOWER(metric_id), value::numeric
  FROM afyabi.mart_household_visit_monthly_area
),

num_sums_std AS (
  SELECT m.metric_id, b.level, b.county, b.sub_county, b.community_unit, b.chp_area, b.period_start, b.month_year,
...
computed_std AS (
  SELECT ns.level, ns.county, ns.sub_county, ns.community_unit, ns.chp_area, ns.period_start, ns.month_year, ns.metric_id,
...
num_chp AS (
  SELECT 'monthly_rates_active_chp'::text AS metric_id, a.level, a.county, a.sub_county, a.community_unit, a.chp_area, a.period_start, a.month_year,
...
den_chp AS (
  SELECT county, sub_county, community_unit, chp_area, value::numeric AS chps_enrolled
  FROM afyabi.mv_aggregate_metrics_summary
  WHERE metric_id = 'chps_enrolled' AND period_id = 1
),

computed_chp AS (
  SELECT n.level, n.county, n.sub_county, n.community_unit, n.chp_area, n.period_start, n.month_year, n.metric_id,
